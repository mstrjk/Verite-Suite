...
package teacommontea.veritechasse.check.impl.autowalk;
...
@CheckInfo(name = "AutoWalkA", description = "Motion tracks a turning body with forward-only input (autowalk).", decay = 0.02)
public final class AutoWalkA extends Check implements PacketCheck, ConfidenceCheck {

    private static final double MOVE_THRESHOLD = 0.12;
    private static final double FORWARD_DOT = 0.995;
    private static final float MIN_YAW_TRAVEL = 35.0f;
    private static final int REQUIRED_TICKS = 80;
    ...
    public AutoWalkA(VeritePlayer player) {
        super(player);
    }
    ...
    public void onPacketReceive(final VeritePacketEvent event) {
        if (event.getPacketType() == VeritePacketType.PLAYER_INPUT) {
            ...
            forwardOnly = forward && !strafing && !event.isBackward();
        ...
        if (exempt || moved < MOVE_THRESHOLD) {
            resetTracking();
        ...
        float yawStep = Math.abs(angleDiff(yaw, lastYaw));
        lastYaw = yaw;

        double yawRad = Math.toRadians(yaw);
        double dot = (dx * -Math.sin(yawRad) + dz * Math.cos(yawRad)) / moved;

        if (!forwardOnly || inputGrace > 0 || dot < FORWARD_DOT) {
            resetTracking();
        ...
        if (trackingTicks > REQUIRED_TICKS && yawTravel >= MIN_YAW_TRAVEL) {
            setInfo("yawTravel=" + formatOffset(yawTravel) + " dot=" + formatOffset(dot)
                    + " fwdOnly=1 ticks=" + trackingTicks);
