...
@CheckInfo(name = "KillAuraD", description = "Yaw not a multiple of the player's learned mouse step.", decay = 0.02)
public final class KillAuraD extends Check implements PacketCheck, ConfidenceCheck {
    ...
    public KillAuraD(VeritePlayer player) {
        super(player);
    }
    ...
    public void onPacketReceive(final VeritePacketEvent event) {
        ...
        if (!r.yawGcd.isCalibrated()) return;
        if (player.currentTick() - player.lastAttackTick > COMBAT_WINDOW) return;

        double d = r.yawDelta;
        if (d < MIN_DELTA || d > MAX_DELTA) return;

        double divergence = r.yawGcd.divergence(d);
        if (divergence > MAX_DIVERGENCE) {
            badStreak++;
            if (badStreak >= SUSTAIN) {
                setInfo("divergence=" + formatOffset(divergence) + " maxDivergence=" + formatOffset(MAX_DIVERGENCE) + " yawDelta=" + formatOffset(d));
